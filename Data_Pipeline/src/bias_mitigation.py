    ...
    for label in counts.index:
        ...
        if len(class_df) < max_count:
            extra = class_df.sample(
                n=max_count - len(class_df), replace=True, random_state=random_state)
...
def stratified_split(df, target_col, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, random_state=42):
    ...
    for label in df[target_col].unique():
        class_df = df[df[target_col] == label].sample(
    ...
    train_df = pd.concat(train, ignore_index=True).sample(
        frac=1, random_state=random_state)
    val_df = pd.concat(val, ignore_index=True).sample(
        frac=1, random_state=random_state)
    test_df = pd.concat(test, ignore_index=True).sample(
        frac=1, random_state=random_state)
    print(
