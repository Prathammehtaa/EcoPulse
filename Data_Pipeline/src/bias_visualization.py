...
| hour_of_day | Hour of day (0-23) | 24 categories |
...
|-----------|-----------------|----------|---------------|
| hour_of_day | 1.00 | LOW | No |
| day_of_week | 1.00 | LOW | No |
| month | 1.00 | LOW | No |
| season | 1.00 | LOW | No |
| is_weekend | 2.50 | MODERATE | No |
| carbon_intensity_bucket | **16.12** | **SEVERE** | **Yes** |

### 4.2 Detailed Analysis - Carbon Intensity Bucket

The most significant bias was found in the carbon_intensity_bucket dimension:
...
We chose random oversampling to balance the carbon_intensity_bucket distribution because:
...
|------|----------|-------------|
| distribution_*.csv | reports/tfdv_bias/ | Distribution stats per slice |
| stats_*.csv | reports/tfdv_bias/ | Statistical summaries |
| bias_summary.json | reports/tfdv_bias/ | Overall bias summary |
| *_distribution.png | reports/tfdv_bias/plots/ | Visualization plots |
...
The EcoPulse dataset exhibited severe class imbalance in the carbon_intensity_bucket dimension (16.12x imbalance ratio). Through random oversampling, we successfully balanced the dataset to a 1.00x ratio while preserving all original data. Stratified splitting ensures consistent representation across train/validation/test sets for fair model evaluation.